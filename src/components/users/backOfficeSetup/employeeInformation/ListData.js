...
import { employeeInformation, userHasAccess } from '../../../../api/userUrl';
...
export default function ListData(props) {
    ...
    const handlePageChange = (pageNumber = 1) => {
        setIsLoading(true);
        // TABLE DATA READY
        userGetMethod(`${employeeInformation}?page=${pageNumber}`)
            .then(response => {
    ...
    const sortHandler = (params) => {
        ...
        let ascUrl = '';
        if (ascDesc === true) {
            ascUrl = `${employeeInformation}?asc=${params}&desc=`;
        } else {
            ascUrl = `${employeeInformation}?asc=&desc=${params}`;
        }
                    ...
                    <div className="col-sm-12 col-md-12 col-lg-12">
                        <div className="card">
                            ...
                            <div className="row">
                                ...
                                <div className="col-md-6 col-lg-6">
                                    <AddButton link="employeeInformation/add" menuId={menuId} />
                                                            ...
                                                            {branchData.map((item, index) =>           
                                                                (
                                                                    <tr key={index}>
                                                                        ...
                                                                        <td>{item.email}</td>
                                                                        <td>{item.working_status == 1 ? 'Working' : ''}</td>
                                                                        <td className="">
                                                                            {
                                                                                accLoad === false ? <>
                                                                                    {hasAccess.edit === true ? <EditButton link={`/employeeInformation/edit/${item.id}`} menuId={ menuId } /> : ''} 
                                                                                    {hasAccess.destroy === true ? <DeleteButton deleteLink={employeeInformation} deleteHandler={ deleteHandler } menuId={ menuId } dataId={item.id} /> : ''} 
                                                                                </> : ''
                                                                            }
                                                                        </td>
                                                                    </tr>
