...
const Edit = (props) => {
    ...
    useEffect(() => {
        ...
        userGetMethod(`${journalVoucherAPI}/${batchMasterId}/edit`)
        .then(response => {
            ...
            response.data.debit_accounts.map( (row, key) => {
            ...
            setDebitTotalAmount(response.data.batch_master.debit_amount);

            response.data.credit_accounts.map( (row, key) => {
                var customRowsObject = {};
                customRowsObject.account_code = row.account_code;
                customRowsObject.account_head = row.account_head;
                customRowsObject.amount = parseInt(row.credit_amount) + parseInt(row.debit_amount);
            ...
            setCreditTotalAmount(response.data.batch_master.credit_amount);
    ...
    const submitHandler = (data, e) => {
        ...
        if( remarksValue != "" && Object.keys(debitTableRow).length > 0 && Object.keys(creditTableRow).length > 0){
            ...
            userPutMethod(`${journalVoucherAPI}/${batchMasterId}`, data )
            .then(response => {
