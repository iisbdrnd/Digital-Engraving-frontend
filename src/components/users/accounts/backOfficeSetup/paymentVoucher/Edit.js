...
import { projectAccountCodeAPI, paymentVoucherAPI } from '../../../../../api/userUrl';
...
const Edit = (props) => {
    ...
    if (props.location.state === undefined) {
        ...
        menuId = props.location.state.params;
    }

    const batchMasterId = props.match.params.id;

    useEffect(() => {
        userGetMethod(`${paymentVoucherAPI}/${batchMasterId}/edit`)
    ...
    const addAmount = (e) => {
        ...
        if(selectedOption != 0  && calculationValue.amount > 0 && calculationValue.amount != "" && typeHeadOptions['paymentBy'].length > 0){
            ...
            if(tableRow.hasOwnProperty(levelIndex)){
                ...
                let remarksVal;
                let defaultValue = "Project"+' '+projectName;
    ...
    const submitHandler = (data, e) => {
        ...
        if(Object.keys(tableRow).length > 0){
            ...
            userPutMethod(`${paymentVoucherAPI}/${batchMasterId}`, data )
            .then(response => {
