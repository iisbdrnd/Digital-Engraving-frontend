...
import {AccountsPayableList} from '../../../../../api/userUrl'
...
const Form = (props) => {
    ...
    useEffect(() => {
        userGetMethod(`${AccountsPayableList}`)
        .then(response => {
    ...
    const submitHandler = (data, e) => {
        ...
        if(reportType == 'upto_date'){
            var url = `${process.env.PUBLIC_URL}/user/accounts-payable-report-action?type=1&branch_id=${branch_id}&`;
            window.open(url, '_blank', 'height=800,width=1200');
        }

        if(reportType == 'date_range'){
            const to_date = data.to_date;
            var url = `${process.env.PUBLIC_URL}/user/accounts-payable-report-action?type=2&branch_id=${branch_id}&to_date=${to_date}&`;
            window.open(url, '_blank', 'height=800,width=1200');
        }
