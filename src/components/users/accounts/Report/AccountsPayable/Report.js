...
import {AccountsPayableReportActionApi} from '../../../../../api/userUrl'
...
const Report = (props) => {
    ...
    const handlePageChange = (pageNumber = 1) => {
        ...
        if(type == 1){
            var url = `${AccountsPayableReportActionApi}?type=1&branch_id=${branch_id}&`;
        }else{
            var url = `${AccountsPayableReportActionApi}?type=2&branch_id=${branch_id}&to_date=${to_date}&`;
        }
    ...
    const TotalBalanceAmount = (credit_amount, debit_amount) => {
        totalBalance += credit_amount - debit_amount;  
    }
                                                                ...
                                                                { Object.values(voucherData).map( (ledger, index) =>  
                                                                    (
                                                                        <>
                                                                            ...
                                                                            {TotalBalanceAmount(ledger.credit_amount,ledger.debit_amount)}

                                                                            <tr>
                                                                                <td style={{textAlign: 'center'}}>{ledger.account_code}</td>
                                                                                <td style={{textAlign: 'center'}}>{chartOfAccountData[ledger.account_code].account_head}</td>
                                                                                <td style={{textAlign: 'center'}}>{ledger.credit_amount-ledger.debit_amount}</td>
                                                                            </tr>
