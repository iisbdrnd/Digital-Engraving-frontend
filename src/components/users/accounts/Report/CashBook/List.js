...
import {cashBookList} from '../../../../../api/userUrl'
...
const Form = (props) => {
    ...
    const submitHandler = (data, e) => {
        ...
        if(reportType == 'upto_date'){
            var url = `${process.env.PUBLIC_URL}/user/cash-book-report-action?type=1&branch_id=${branch_id}&account=${account}&`;
        ...
        if(reportType == 'date_range'){
            ...
            var url = `${process.env.PUBLIC_URL}/user/cash-book-report-action?type=2&branch_id=${branch_id}&account=${account}&from_date=${from_date}&to_date=${to_date}&`;
            window.open(url, '_blank', 'height=800,width=1200');
        }
    }
