...
class UserChangeImage extends Component {
    constructor(props) {
        super(props);
    ...
    uploadImages = (e) => {
        ...
        axios.post(`${process.env.REACT_APP_BASEURL}/api/user/profileImageUpload`,formData,{ headers: {"Authorization" : `Bearer ${userToken}`} })
        .then(response=>{       
            if (response.data.status == 1) {
                this.props.history.push(`${process.env.PUBLIC_URL}/userEdit`);
