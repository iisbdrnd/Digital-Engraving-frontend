...
import {PENDING_SUPPLY_INFORMATION_FORM} from '../../../../api/userUrl'
...
const Form = (props) => {
    ...
    useEffect(() => {
        userGetMethod(`${PENDING_SUPPLY_INFORMATION_FORM}`)
        .then(response => {
    ...
    const submitHandler = (data, e) => {
        ...
        const pending_category = data.pending_category;
        console.log('from_date ', data);
        let url = `${process.env.PUBLIC_URL}/pendingSupplyInformation/${data.from_date}/${data.to_date}/${data.employee_id ? data.employee_id : null}/${data.pending_category ? data.pending_category : null}`;
                        ...
                        <div className="card">
                            <div className="card-header">
                                <div className="row">
                                    <div className="col-md-6">
                                        <h5>Pending Supply Information</h5>
                                    </div>
                                </div>
                            </div>
                            <div className="card-body">
                                <form onSubmit={handleSubmit(submitHandler)} className="needs-validation theme-form">
                                    ...
                                    <div className="form-group row">
                                        <label className="col-sm-3 col-form-label" htmlFor="pending_category">Pending</label>
                                        <div className="col-sm-3">
                                            <select 
                                            ...
                                            {errors.pending_category && <p className='text-danger'>{errors.pending_category.message}</p>}
