...
import { DE_CHROME_RS_URL, CHECK_DE_CHROME_CYL_EXIST_OR_NOT, GET_DE_CHROME_CYLINDERS_BY_JOB_ID } from '../../../../api/userUrl';
...
export default function TankScheduleEdit(props) {
    ...
    useEffect(() => {
        ...
        userGetMethod(`${DE_CHROME_RS_URL}/${props.chromeScheduleMasterId}/edit`)
            .then(response => {
                // FOR JOB ORDER
    ...
    const dropDownChange = (event, stateName) => {
        if(event.length > 0){
            ...
            userGetMethod(`${GET_DE_CHROME_CYLINDERS_BY_JOB_ID}?jobId=${selectedValueId}`)
                .then(response => {
    ...
    const submitHandler = (data, e) => {
        ...
        if (data.cylScheduleArr.length > 0) {
            userPutMethod(`${DE_CHROME_RS_URL}/${props.chromeScheduleMasterId}`, data)
                .then(response => {
