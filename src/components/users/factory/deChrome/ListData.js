...
export default function ListData(props) {
    ...
    let interval = useRef();
    const startTimer = (startDate, baseDate, deChromeMastersId) => {
        // START TIME FORMATION
        let startDateFinal = new Date(startDate);
        let formattedStartDate = format(startDateFinal, "MMM d, yyyy HH:mm:ss");
        // EST END TIME FORMATION
        let baseDateFinal = new Date(baseDate);
        let formattedDate = format(baseDateFinal, "MMM d, yyyy HH:mm:ss");
        const counterTime = new Date(formattedDate).getTime(); //Nov 1, 2021 18:00:00
        // BASE OF TOTAL SECONDS STANDS ON START TIME & EST END TIME
        const base_total_seconds = Math.round(Math.abs(new Date(formattedStartDate).getTime() - new Date(formattedDate).getTime()) / 1000);
        ...
        interval = setInterval(() => {
            ...
            let now_total_seconds = Math.round(Math.abs(new Date().getTime() - new Date(formattedDate).getTime()) / 1000);
            let per_percent_less_val = Math.round(base_total_seconds * 0.01); // 1% = HOW MUCH SECONDS
            let completed_percentage = Math.round((base_total_seconds - now_total_seconds)/per_percent_less_val); // COMPLETED_PERCENTAGE
            ...
            .then(response => {
                ...
                if (response.data.deChromeMasters && response.data.deChromeMasters.length > 0) {
                    response.data.deChromeMasters.map(scheduleMaster => {
                        if (scheduleMaster.est_end_time != null  && scheduleMaster.complete_status == 1) {
                                                    ...
                                                    <div className="card-header">
                                                        <div className="row">
                                                            ...
                                                            <div className="col-md-6">
                                                                ...
                                                                {schedule.complete_status == 1 && schedule.est_end_time != null ? 
                                                                    <>
                                                                        <span className="btn btn-success btn-sm m-r-10">
                                                                            {clockData['clock_'+schedule.id] ? clockData['clock_'+schedule.id] : '00:00:00'}
                                                                            </span> 
                                                                        {/* <span className="" style={{width: '250px', float: 'right', marginTop: '7px'}}>
                                                                            <ProgressBar animated striped variant="success" now={progressVal} label={`${progressVal}%`} style={{backgroundColor: 'gray'}} />
                                                                        </span> */}
                                                                    </>
                                                                : ''}
                                                            </div>
