...
export default function TankSchedule(props) {
    ...
    useEffect(() => {
        ...
        userGetMethod(`${CHROME_RS_URL}/create`)
            .then(response => {
                // FOR JOB ORDER
                let jobOrderOptions = [];
    ...
    const addCylScheduleHandler = (event) => {
        ...
        if (dropdownData.job_order_id === ''|| fl === '' || cir === '' || dia === '' || cylinder_id === '' || job_type == '' || est_plating_order === '' || surface_area === '') {
            ...
            if (check){
                userGetMethod(`${CHECK_CHROME_CYL_EXIST_OR_NOT}?cylinder_id=${cylinder_id}`)
                    .then(response => {
                        console.log("response.data", response.data);
                        if (response.data.chromeExistence != 0) {
    ...
    return (
        <Modal isOpen={ props.modal && isOpenModalPrev } toggle={props.toggle} size="xlg">
            <ModalHeader toggle={props.toggle}>Manual Cycle Plan Form #{props.modalTitle} Thank #chrome-add</ModalHeader>
            <ModalBody>
