...
import { CHROME_RS_URL, CHECK_CHROME_CYL_EXIST_OR_NOT, GET_CYLINDERS_BY_JOB_ID } from '../../../../api/userUrl';
...
export default function TankScheduleEdit(props) {
    ...
    useEffect(() => {
        ...
        userGetMethod(`${CHROME_RS_URL}/${props.chromeScheduleMasterId}/edit`)
            .then(response => {
                // FOR JOB ORDER
    ...
    const dropDownChange = (event, stateName) => {
        if(event.length > 0){
            ...
            userGetMethod(`${GET_CYLINDERS_BY_JOB_ID}?jobId=${selectedValueId}`)
                .then(response => {
    ...
    const addCylSchedulHandler = (event) => {
        ...
        if (dropdownData.job_order_id === ''|| fl === '' || cir === '' || dia === '' || cylinder_id === '' || job_type == '' || est_plt_order === '' || surface_area === '') {
            ...
            if (check){
                userGetMethod(`${CHECK_CHROME_CYL_EXIST_OR_NOT}?job_order_id=${dropdownData.job_order_id}&cylinder_id=${cylinder_id}`)
                    .then(response => {
    ...
    const submitHandler = (data, e) => {
        ...
        if (data.cylScheduleArr.length > 0) {
            userPutMethod(`${CHROME_RS_URL}/${props.chromeScheduleMasterId}`, data)
                .then(response => {
